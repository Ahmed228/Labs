...
int readamount(FILE *in){
	int c = 0; int amount = 0; int noteof = 0; int minus = 0;
	c = fgetc(in);
	while ((c != '\n') && (c != EOF)){
		noteof++;
		if (c == '-') minus++;
...
int getgraph(int *graph, FILE *in, int vert, int *res){
	int minus = 0; int lines = 3; int errors = 0; int c = fgetc(in); int counter = 0;
	while (c != EOF) {
		int from = 0; int to = 0; int len = 0;
		minus = 0;
		while ((c != ' ') && (c != EOF) && (c != '\n')){
			if (c == '-') minus++;
...
int findmin(int *path, char *crossed, int vert){   //
	int i = 0; int min = -1; int existance = 0;
	for (i; i < vert; i++) if ((path[i] >= 0) && (crossed[i] == 0)) 
	                           if (existance == 0) {
				                   min = i;
				                   existance = 1;
			                   } else if (path[i] < path[min]) min = i;
	return min + 1;
}
...
void deikstra(int *graph, int *path, char *crossed, int edges, int vert, int start, int finish, FILE *out){
	...
	if (path[finish - 1] == -1) fprintf(out, "no path");
		...
		if (errors == 1) fprintf(out, "overflow");
			...
			i = 0;
			for (i; i <= j; i++) fprintf(out, "%d ", used[i]);
		}
	}
}
...
int main(){
	...
	assert(in != NULL);
	int vert = readamount(in);
	if ((vert < 0) || (vert > 5000)) fprintf(out, "bad number of vertices");
	else {
		int start = 0; int finish = 0;
		readpath(in, &start, &finish);
		int edges = readamount(in);
		if ((edges < 0) || (edges > vert*(vert - 1) / 2)) fprintf(out, "bad number of edges");
		else {
			...
			int res = 0; int errors = 0;
			int lines = getgraph(graph, in, vert, &res);
			...
			if (errors == 0) deikstra(graph, path, crossed, edges, vert, start, finish, out);
			free(graph);
			free(path);
			free(crossed);
		}
